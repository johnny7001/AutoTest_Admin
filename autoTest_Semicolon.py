...
import logging
import time
from pkg import open_driver, AdminLogin, cookieLogin, sendEmail
...
try:
    ...
    if alert.text != '更新成功':
        ...
        mailContent += f'彈跳視窗回傳訊息: {alert.text},請檢查是否異常-Fail\n'
    else:
...
if contactEmail == editContactEmail:
    ...
    mailContent += '信箱內容未被修改,請檢查是否異常-Fail\n'
else:
    print('信箱內容更新成功-Pass')
    logging.info('信箱內容更新成功-Pass')
    mailContent += '信箱內容更新成功-Pass\n'
logging.info("="*150)
driver.close()
# 寄信通知
